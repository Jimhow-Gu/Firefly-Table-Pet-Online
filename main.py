...
class MultiplayerClient(QObject):
    ...
    def __init__(self, firefly, parent=None):
        ...
        self.firefly = firefly          # 必须保存 firefly 引用
        self.sock = None
        ...
        try:
            ...
            login_msg = {
                ...
                'data': {
                    ...
                    'country': getattr(self, 'login_country', '')  # 新增
                }
            }
    ...
    def _send(self, obj):
        ...
        try:
            ...
            print(f"[发送失败] {e}")
            self.disconnect()  # 主动断开，避免重复尝试

    def send_chat(self, text):
        self._send({'type': 'chat', 'data': {'text': text}})

    def send_command(self, cmd):
        self._send({'type': 'command', 'data': {'command': cmd}})

    def request_player_list(self):
        self._send({'type': 'get_player_list', 'data': {}})

    def disconnect(self):
        self.connected = False  # 先标记断开
        if self.sock:
            try:
                self._send({'type': 'disconnect', 'data': {}})
    ...
    def _process_message(self, line):
        ...
        if mtype == 'mention':
            ...
            self.login_success.emit(self.server_uid, self.server_permission, inv)
...
class Firefly(QWidget):
    ...
    def on_command_result(self, success, message):
        if success:
            ...
            if '改名成功' in message:
                ...
                self.mp_client._send({'type': 'get_chat_history'})  # 请求最新历史
        else:
    ...
    def refresh_chat_window(self):
        if self.chat_window:
            try:
                ...
                pass
            self.chat_window = None
        QTimer.singleShot(300, self._safe_open_chat)
    ...
    def on_login_success_mp(self, uid, perm, inv):
        self.connecting = False
        ...
        if inv:
            ...
            self.online_inventory = {'balance': 0, 'credit': 0, 'owned_clothes': ['normal'], 'star_rail_tickets': 0}
        self.update_bag_display()
        QTimer.singleShot(100, lambda: self.show_centered_message("成功", f"已连接到服务器\n你的服务器UID: {uid}"))
        QTimer.singleShot(200, self.create_menu)

    def on_mp_login_failed(self, reason):
        self.connecting = False
        if "封禁" in reason:
            self.banned = True
            # 直接弹窗，不再发射 banned_by_server 信号
    ...
    def open_chat_window(self):
        if self.banned:
            self.show_centered_message("提示", "你已被封禁，无法使用聊天功能", QMessageBox.Warning)
            return
        if not self.mp_client.connected:
            return
        if self.chat_window and self.chat_window.isVisible():
    ...
    def join_server(self, host, port):
        ...
        country = 'UNKNOWN'
        try:
            # 使用 ipapi.co 获取国家代码（ISO 3166-1 alpha-2，如 CN、US）
            with urllib.request.urlopen('https://ipapi.co/country/', timeout=3) as response:
...
"""
    ...
    def contextMenuEvent(self, event):
        ...
        try:
            ...
            action0 = QAction('Firefly_Win64_v4.3.5_G', self)
            ...
            # ❌ 删掉这三行：balance = self.current_user.get(...)
    ...
    def create_menu(self):
        ...
        menu = RoundedMenu()
        menu.setToolTipsVisible(True)
        fuck_Manthe = menu.addAction("Firefly_Win64_v4.3.5_G")
